var BackTraffic = async function (){
    ...
    var From_account = new Array();
    From_account = await web3.eth.getAccounts();

    console.timeEnd("getAccounts");

    console.time("unlockAccount");

    web3.eth.personal.unlockAccount(From_account[0], "123", 10000000);
    ...
    function selectGasPrice() {
        ...
        // 遍历 gasPriceDistribution，累加概率，找到第一个超过随机数的 gasPrice
        ...
        // 如果未选择到 gasPrice，返回一个默认值
        return 32; // 默认 gasPrice
    }
    console.time("sendTransaction");

    //for (var i = 0; i < Math.round(Math.random()*8); i++){
    for (var i = 0; i < 4; i++){
        var Rd_gasPrice = selectGasPrice();
        rl.oneline('./account.txt', i+1, function(err,res){ 
            if (err) console.error(err);
            web3.eth.sendTransaction({from: From_account[0], to: res, value: web3.utils.toWei('0','ether'), gasPrice: Rd_gasPrice*1000000000},(error, result)=>{
                if(error)
